...
from torch import set_printoptions,hub,Tensor,from_numpy,tensor,zeros,cat,mm,load,save
...
# Settings
set_printoptions(linewidth=320, precision=5, profile='long')
...
def check_file(file):
    ...
    if Path(file).is_file() or file == '':  # exists
        ...
        hub.download_url_to_file(url, file)
    ...
    def download_one(url, dir):
        ...
        if not f.exists():
            ...
            if curl:
                ...
                hub.download_url_to_file(url, f, progress=True)  # torch download
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    t = time.time()
    output = [zeros((0, 6), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            v = zeros((len(l), nc + 5), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
        ...
        if multi_label:
            ...
            x = cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 5:].max(1, keepdim=True)
            x = cat((box, conf, j.float()), 1)[conf.view(-1) > conf_thres]
...
def strip_optimizer(f='best.pt', s=''):  # from utils.general import *; strip_optimizer()
    ...
    x = load(f, map_location= Device('cpu'))
    if x.get('ema'):
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = model(Tensor(ims).to(d.device)).argmax(1)  # classifier prediction
