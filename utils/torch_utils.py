...
# import cuda as cuda
from torch import manual_seed,Tensor,float16,mm,sqrt,zeros,diag,no_grad
...
def select_device(device='', batch_size=None):
    ...
    s = f'YOLOv5 🚀 {git_describe() or date_modified()} torch {torch_version} '  # string
    cpu = device.lower() == 'cpu'
    if cpu:
        os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # force torch.cuda.is_available() = False
    elif device:  # non-cpu device requested
        os.environ['CUDA_VISIBLE_DEVICES'] = device  # set environment variable
        assert Cuda.is_available(), f'CUDA unavailable, invalid device {device} requested'  # check availability

    cuda = not cpu and Cuda.is_available()
    if cuda:
...
def profile(x, ops, n=100, device=None):
    ...
    # print(torch_version, device.type, Cuda.get_device_properties(0) if device.type == 'cuda' else '')
...
def load_classifier(name='resnet101', n=2):
    ...
    model.fc.bias = nn.Parameter(zeros(n), requires_grad=True)
    model.fc.weight = nn.Parameter(zeros(n, filters), requires_grad=True)
